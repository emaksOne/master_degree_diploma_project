...
def buildPetuninEllipses(pointsDict):
 ...
 178692 ,186671]
    ...
    transrormationMatrix = np.linalg.inv(transrormationMatrix)

    #pointsInOriginBases = applyMatrixToPoints(map(lambda x: x[0], pointsDict), transrormationMatrix)
    pointsInOriginBases = applyMatrixToPoints(pointsDict, transrormationMatrix)

    plotPoints(map(lambda x:pointsInOriginBases[x], convexVerts), 'go')
...
def applyMatrixToPoints(pointInfos, matrix):
    newPointsInfos = []
    i = 0
    for info in pointInfos:
        #print i
        #i+=1
        #newPoints.append(np.dot(matrix, p))
        point = info[0]
        newPointsInfos.append((np.dot(matrix, point), info[1]))
